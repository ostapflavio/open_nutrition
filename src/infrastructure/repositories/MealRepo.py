...
from app.database_models import (
    Meal as MealEntity,
    MealEntry as MealEntryEntity,
    FavoriteMeal as FavoriteMealEntity
)
...
class MealRepository:
    ...
    def find_by_name(self, query: str, limit: int) -> list[Meal]:
        ...
        ents = (
            self.session
            .query(MealEntity)
            .filter(MealEntity.name.ilike(f"%{query}%"))
            .limit(limit)
            .all()
        )
        return [self.to_domain(e) for e in ents]

    def create(self, domain_meal: Meal) -> Meal:
        # 1) Persist the Meal itself
        ent = MealEntity(
            name=domain_meal.name,
            timestamp=domain_meal.timestamp,
            #is_favourite=domain_meal.is_favourite,
        )
    ...
    def update(self, meal_id: int, domain_meal: Meal) -> Meal:
        ...
        ent.timestamp    = domain_meal.timestamp
        #ent.is_favourite = domain_meal.is_favourite
